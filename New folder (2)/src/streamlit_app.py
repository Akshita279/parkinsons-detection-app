...
class AdvancedParkinsonsDetector:
    ...
    def convert_audio_to_wav(self, uploaded_file):
        ...
        try:
            ...
            # Clean up original temp file (but keep WAV)
    ...
    def load_and_train_advanced_model(self):
        """Load data and train advanced model with comprehensive dataset"""
        try:
            # Try to load the latest trained model first
    ...
    def predict_from_features(self, features):
        ...
        try:
            ...
            if len(features) != 22:
                print(f"Expected 22 features, got {len(features)}")
                return None
            ...
            # Get SHAP values for explanation if explainer exists
            shap_values = None
            if self.explainer:
                try:
    ...
    if analysis_mode == "Voice Recording Analysis":
                ...
                with st.spinner("Analyzing voice patterns..."):
                    try:
                        ...
                        if wav_path and os.path.exists(wav_path):
                            ...
                            # Clean up temporary file after feature extraction
                            try:
                                os.unlink(wav_path)
                            except:
                            ...
                            if result:
                                ...
                                if 'shap_values' in result and result['shap_values'] is not None:
                                    try:
                                        ...
                                        st.info("**Alternative Explanation:**")
                                        
                                        # Show basic feature analysis instead
                                        if result['prediction'] == 'Parkinson\'s Disease':
                                            st.write("**Factors suggesting Parkinson's:**")
                                            st.write("• Voice instability patterns detected")
                                            st.write("• Abnormal jitter/shimmer values")
                                            st.write("• Reduced voice quality metrics")
                                        else:
                                            st.write("**Factors suggesting healthy voice:**")
                                            st.write("• Stable voice patterns")
                                            st.write("• Normal jitter/shimmer values")
                                            st.write("• Good voice quality metrics")
                                else:
                                    st.info("**Model Explanation:**")
                                    if result['prediction'] == 'Parkinson\'s Disease':
                                        st.write("🔴 **High Parkinson's Probability Detected**")
                                        st.write("The model identified patterns in your voice that are similar to those found in Parkinson's patients:")
                                        st.write("• **Voice instability** - Irregular pitch or volume")
                                        st.write("• **Increased jitter** - Pitch variations")
                                        st.write("• **Increased shimmer** - Amplitude variations")
                                        st.write("• **Reduced voice quality** - Noise-to-signal ratio")
                                        
                                        st.warning("**Important:** This could be due to:")
                                        st.write("• Recording quality differences")
                                        st.write("• Casual vs clinical recording conditions")
                                        st.write("• Audio compression artifacts")
                                        st.write("• Individual voice characteristics")
                                    else:
                                        st.write("✅ **Healthy Voice Pattern Detected**")
                                        st.write("The model found voice characteristics similar to healthy individuals:")
                                        st.write("• **Stable voice patterns**")
                                        st.write("• **Normal pitch control**")
                                        st.write("• **Good voice quality**")
                                        st.write("• **Low noise levels**")
                        else:
                            st.error("Failed to convert audio file. Please try a different format.")
        ...
        if st.button("🔍 Analyze Features", type="primary"):
            ...
            if result:
                ...
                # Feature explanation
                st.subheader("📋 Feature Values Summary")
        ...
        try:
            # Try to load large-scale dataset first
            if Path("data/processed/large_scale_parkinsons_dataset.csv").exists():
                df = pd.read_csv("data/processed/large_scale_parkinsons_dataset.csv")
                dataset_name = "Large-Scale Dataset"
            else:
                df = pd.read_csv("data/raw/uci_parkinsons_original.data")
                dataset_name = "UCI Original Dataset"
            
            col1, col2 = st.columns(2)
            
            with col1:
                st.subheader(f"📊 {dataset_name} Overview")
                st.write(f"**Total Samples:** {len(df):,}")
                st.write(f"**Parkinson's Cases:** {df['status'].sum():,}")
                st.write(f"**Healthy Cases:** {len(df) - df['status'].sum():,}")
                st.write(f"**Features:** {len([col for col in df.columns if col not in ['name', 'status', 'source']])}")
                ...
                if 'source' in df.columns:
                    ...
                    for source, count in source_counts.items():
                        st.write(f"  • {source}: {count:,} samples")
                
                # Class distribution
                fig, ax = plt.subplots(figsize=(6, 4))
                class_counts = df['status'].value_counts()
                bars = ax.bar(['Healthy', 'Parkinson\'s'], [class_counts[0], class_counts[1]], 
                            color=['green', 'red'], alpha=0.7)
            ...
            with col2:
                ...
                # Feature importance
                if detector.model and hasattr(detector.model, 'feature_importances_'):
                    ...
                    st.pyplot(fig)
                elif detector.model and hasattr(detector.model, 'estimators_'):
                    # For ensemble models, try to get feature importance from first estimator
                    try:
                        if hasattr(detector.model.estimators_[0], 'feature_importances_'):
                            feature_importance = pd.DataFrame({
                                'Feature': detector.feature_names,
                                'Importance': detector.model.estimators_[0].feature_importances_
                            }).sort_values('Importance', ascending=False).head(10)
                            
                            fig, ax = plt.subplots(figsize=(8, 6))
                            sns.barplot(data=feature_importance, y='Feature', x='Importance', ax=ax)
                            ax.set_title('Top 10 Feature Importance (First Estimator)')
                            st.pyplot(fig)
                        else:
            ...
            numeric_cols = [col for col in numeric_cols if col != 'status']  # Exclude target
            ...
            st.error("Dataset not found. Please run data collection first.")
        except Exception as e:
